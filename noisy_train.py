...
#tf.flags.DEFINE_string("data_file", "./data/reuter.txt", "Data source for the training data.")
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        for grad,var in grads_and_vars:
            if grad is not None:
                if isinstance(grad, ops.IndexedSlices):
                    ...
                    grad_values = grad_values+np.random.normal(loc=0,scale=4)
                    grad = ops.IndexedSlices(grad_values, grad.indices, grad.dense_shape)
                else:
                    grad_values = grad
                    grad_values = grad_values+np.random.normal(loc=0,scale=4)
            grad_noise.append((grad,var))
        train_op = optimizer.apply_gradients(grad_noise, global_step=global_step)
        #grads_noise=[g+np.random.normal(loc=0,scale=4) for g in grads_float] 
        #train_op = optimizer.apply_gradients(zip(grad_noise,var_list), global_step=global_step)
        ...
        file = open('model_dir.txt', 'w')#chen
        file.write(out_dir)#chen
        file.close()#chen
        print("Writing to {}\n".format(out_dir))
        ...
        def dev_step(x_batch, y_batch,writer=None):
            ...
            step, summaries, loss, accuracy = sess.run(
                ...
                feed_dict)
            time_str = datetime.datetime.now().isoformat()
            result = "{}: step {}, loss {:g}, acc {:g}".format(time_start-time_str, step, loss, accuracy)
            print(result)
